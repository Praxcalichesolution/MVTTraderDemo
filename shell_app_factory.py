...
def _render_index(mode: str) -> str:
    ...
    config["api_base"] = os.getenv("RADIANT_API_BASE", "/api")
...
async def _proxy_api_request(full_path: str, request: Request) -> Response:
    backend_host = os.getenv("RADIANT_BACKEND_HOST", "localhost")
    backend_port = os.getenv("RADIANT_BACKEND_PORT", "8000")
    query = f"?{request.url.query}" if request.url.query else ""
    url = f"http://{backend_host}:{backend_port}/api/{full_path}{query}"
...
def create_shell_app(mode: str) -> FastAPI:
    ...
    async def api_proxy(full_path: str, request: Request):
        return await _proxy_api_request(full_path, request)
